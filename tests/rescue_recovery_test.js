...
// Compact local safety backups intentionally exclude historical archives. Restoring one
// through rescue must preserve the real archive store instead of overwriting it with {}.
storage.setItem('vocab_machine_round_archives_v1', JSON.stringify({ archived: { id: 'archived', name: '第一轮' } }));
const compactRestoreOps = api.buildRestoreOperations(api.normalizePayload({
  round: { generation: 1, roundId: 'round-1', startedAt: '2026-09-02T20:00:00.000+08:00' },
  archives: {},
  marks: { book: { known: [7], unknown: [] } }
}), { key: 'backup:compact', payloadHash: 'compact', archiveHistoryExcluded: true });
const archiveRestoreOp = compactRestoreOps.find((op) => op.key === 'vocab_machine_round_archives_v1');
assert(archiveRestoreOp, 'compact backup restore must still emit archive-store operation');
assert.deepStrictEqual(JSON.parse(archiveRestoreOp.value), { archived: { id: 'archived', name: '第一轮' } }, 'compact backup restore must preserve the real archive history');
