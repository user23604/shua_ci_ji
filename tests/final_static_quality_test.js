...
jsFiles.filter((file) => file !== 'app.bundle.js').forEach((file) => {
  const source = read(path.join('assets/js', file));
  const lines = source.split(/\r?\n/).length;
  assert(lines <= 500, `source module is too long for reliable review: ${file} (${lines} lines)`);
  for (const match of source.matchAll(/^function\s+([A-Za-z_$][\w$]*)\s*\(/gm)) {
    const name = match[1];
    assert(!functionOwners.has(name), `duplicate global function ${name}: ${functionOwners.get(name)} and ${file}`);
    functionOwners.set(name, file);
  }
});
assert(functionOwners.size >= 500, 'unexpectedly small global function scan');
