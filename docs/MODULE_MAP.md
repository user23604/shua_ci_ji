...
本文档以 `2026-07-26-final-stable-v3` 为准。实际加载顺序以 `app.js` 为唯一事实来源。
...
| `02-storage-basic.js` | localStorage 安全读写、配额清理、权威 JSON 损坏隔离和恢复锁触发。 |
...
| `15-setup-events.js` | 设置交互、云配置验证、备份和排查包导出。 |
| `16-study-start.js` | 建立学习队列并进入刷词页。 |
| `17-flashcard-render.js` | 单词卡渲染。 |
| `18-speech.js` | Web Speech 朗读和语音状态。 |
| `19-gesture.js` | 点击、滑动等手势。 |
| `20-study-flow.js` | 下一词、暂停、标记、撤销和轮次流程。 |
| `21-archive-stats.js` | 词库和统计页面。 |
...
| `08b-sync-hash-status.js` | dirty/clean 判定、远端确认状态、硬恢复锁设置/验证解除和状态迁移。 |
...
| 文件 | 职责 |
|---|---|
| `sw.js` | 原子安装同源程序壳；GitHub API/raw 完全绕过缓存。 |
