...
2. `app.js` 是小型加载器，按顺序动态创建 `<script>` 标签加载 `assets/js/` 下 33 个 JS 文件
...
### 13-activity.js
学习统计采集：recordStudyActivity、commitCurrentCardActivity、collectActivityStats 等。
...
同步状态计算与云配置校验：computeSyncStatus、validateSavedCloudConfig、savedCloudConfigGate、bootstrapSyncAfterInit、initializeP0Sync 等。
只读模式下增加 `local_only` / `cloud_loaded` 状态细分，避免只读空本地误显示为异常。
...
| 问题 | 说明 |
|---|---|
| `isStrictlyEmptyLocalPayload` 重复定义 | 08 和 09 各有一个定义，后者覆盖前者（与拆分前行为一致）。后续同步修复时统一。 |
...
| 部分 sync 文件仍较大 | 08-sync-hash-state.js (~500行) 后续可进一步拆分 |
