...
- [ ] 硬恢复锁只能由经过哈希验证的回滚/备份或独立救援确认解除；禁止以“payload 非空”作为解除条件。
- [ ] 权威业务 JSON 解析失败必须隔离原文并阻断同步，禁止静默回退为空。
...
另执行：JavaScript 语法扫描、全局函数重复扫描、版本一致性、manifest 哈希和 ZIP 完整性检查。必须覆盖回滚本身失败后重启、历史 blocker 后 PATCH 验证成功、备份优先级、本地 JSON 损坏隔离和活动草稿持久化顺序。

## 必做真机场景

- [ ] 中国大陆移动/宽带：不开 VPN、开启至少两个 VPN 节点分别测试匿名读和 PATCH。
- [ ] 手机学习后平板继续，反向再测一次。
- [ ] 手机学习并确认云端保存后平板继续，反向再测一次。
- [ ] 可选并发回归：两设备从同一远端版本分别修改不同标记后连续上传，确认两边记录最终都保留。
- [ ] 同一单词相反标记，确认较新操作胜出。
