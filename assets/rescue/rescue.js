(function() {
  ...
  const BUSINESS_PREFIXES = [SETTINGS_KEY, "vocab_machine_sync_meta_v1", HASH_SYNC_STATE_KEY, HASH_BACKUP_PREFIX, DAILY_BACKUP_PREFIX, "vocab_machine_sync_audit_v1", "progress:", "unknown_progress:", "marks:", "activity:", "unit_stats:", LOCAL_SNAPSHOT_KEY, "shua_ci_ji_sync_lock"];
  let lastBundle = null;
  ...
  function classifyCandidate(item) {
    const c = { key: item.key, kind: "missing", savedAt: item.meta.savedAt || "", payloadHash: "", parseOk: false, validateOk: false, nonEmpty: false, progressCount: 0, marksCount: 0, activityDayCount: 0, reason: "", payload: null, raw: item.raw };
  ...
  function buildBundle() {
    ...
    const candidates = collectBackupCandidates().map(classifyCandidate);
    return {
  ...
  function buildSummary(bundle) {
    const stateRaw = localStorage.getItem(HASH_SYNC_STATE_KEY);
    const syncState = parseJson(stateRaw || "{}", {});
    ...
    lines.push("");
    lines.push("备份候选：");
    bundle.candidates.forEach(c => lines.push("- " + c.kind + " | " + c.key + " | savedAt=" + (c.savedAt || "无") + " | hash=" + (c.payloadHash || "无") + " | progress=" + c.progressCount + " marks=" + c.marksCount + " activityDays=" + c.activityDayCount + " | " + (c.reason || "")));
  ...
  function renderCandidates(candidates) {
    ...
    const rows = candidates.map((c, index) => '<tr>' +
      ...
      '<td>progress ' + c.progressCount + '<br>marks ' + c.marksCount + '<br>activity ' + c.activityDayCount + '</td>' +
      ...
      '</tr>').join('');
    $("candidateTable").innerHTML = '<table><thead><tr><th>key</th><th>kind</th><th>savedAt</th><th>payloadHash</th><th>parse</th><th>valid</th><th>nonEmpty</th><th>摘要</th><th>原因</th><th>操作</th></tr></thead><tbody>' + rows + '</tbody></table>';
  }
  ...
  function restoreCandidate(index) {
    ...
    const stateRaw = parseJson(localStorage.getItem(HASH_SYNC_STATE_KEY) || "{}", {});
