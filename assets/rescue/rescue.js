(function() {
  ...
  const BUSINESS_PREFIXES = [CLOUD_KEY, SETTINGS_KEY, "vocab_machine_sync_meta_v1", HASH_SYNC_STATE_KEY, HASH_SYNC_STATE_KEY_V1, HASH_BACKUP_PREFIX, DAILY_BACKUP_PREFIX, "vocab_machine_sync_audit_v1", PROGRESS_CURSOR_KEY, UNKNOWN_PROGRESS_CURSOR_KEY, PROGRESS_PENDING_KEY, ACTIVITY_DRAFT_KEY, ROUND_STATE_KEY, ROUND_ARCHIVES_KEY, "progress:", "unknown_progress:", "marks:", "mark_states:", "activity:", "unit_stats:", LOCAL_SNAPSHOT_KEY, "shua_ci_ji_sync_lock"];
  let lastBundle = null;
  ...
  function extractPayload(obj) {
    ...
    for (const item of candidates) {
      ...
      if (item.progress || item.marks || item.markStates || item.activity || item.unitStats || item.unknownProgress || item.settings || item.round || item.archives) return normalizePayload(item);
    }
    return null;
  }
  function normalizePayload(p) {
    ...
    return {
      ...
      round: isPlainObject(src.round) ? src.round : { generation: 0, roundId: "legacy-0", startedAt: "" },
      archives: isPlainObject(src.archives) ? src.archives : {},
  ...
  function countArchives(p) { return Object.keys(isPlainObject(p.archives) ? p.archives : {}).length; }
  function hasBusinessData(p) {
    const generation = Math.max(0, Number(p && p.round && p.round.generation) || 0);
    return countProgress(p) > 0 || countMarks(p) > 0 || countActivity(p) > 0 || countStudyState(p) > 0 || generation > 0 || countArchives(p) > 0;
  }
  ...
  function collectBackupCandidates() {
    ...
    // Backup index is best-effort metadata and may be dropped during quota recovery.
    // Scan physical hash-backup keys as well so rescue can still discover them.
    try {
  ...
  function classifyCandidate(item) {
    const c = { key: item.key, kind: "missing", savedAt: item.meta.savedAt || "", payloadHash: "", parseOk: false, validateOk: false, nonEmpty: false, progressCount: 0, marksCount: 0, activityDayCount: 0, studyStateCount: 0, archiveCount: 0, score: 0, reason: "", payload: null, raw: item.raw };
    ...
    c.payload = payload;
    c.archiveHistoryExcluded = parsed.archiveHistoryExcluded === true;
    c.savedAt = parsed.savedAt || parsed.timestamp || item.meta.savedAt || "";
    c.payloadHash = shortHash({ settings: payload.settings, round: payload.round, archives: payload.archives, progress: payload.progress, unknownProgress: payload.unknownProgress, marks: payload.marks, markStates: payload.markStates, activity: payload.activity, unitStats: payload.unitStats });
    ...
    c.archiveCount = countArchives(payload);
    c.score = c.progressCount * 10 + c.marksCount * 3 + c.activityDayCount * 2 + c.studyStateCount * 5 + c.archiveCount * 20;
  ...
  function buildRestoreOperations(payload, candidate) {
    const p = payload || {};
    const existingArchives = parseJson(storageGet(ROUND_ARCHIVES_KEY) || "{}", {});
    const archivesForRestore = candidate && candidate.archiveHistoryExcluded === true
      ? (isPlainObject(existingArchives) ? existingArchives : {})
      : (p.archives || {});
    const operations = [
      { key: SETTINGS_KEY, value: JSON.stringify(p.settings || {}) },
      { key: ROUND_STATE_KEY, value: JSON.stringify(p.round || { generation: 0, roundId: "legacy-0", startedAt: "" }) },
      { key: ROUND_ARCHIVES_KEY, value: JSON.stringify(archivesForRestore) }
    ];
