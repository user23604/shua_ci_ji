...
async function syncBranchMerge({ remote, remotePayload, local, keepalive, reason, runId, rebaseCount = 0, patch409Retries = 0 }) {
  ...
  const result = await patchBusinessPayloadToGist(mergedPayload, { remote, keepalive, runId, reason, remoteHashAtDecision: currentRemoteHash(remote) });
  if (isStaleSyncRun(runId)) return false;
  // 409 retryable conflict: 延迟后重试，不超过 MAX_PATCH_409_RETRIES
  if (result.retryableConflict) {
    if (patch409Retries >= MAX_PATCH_409_RETRIES) {
  ...
  try {
    ...
    try {
      ...
      if (reason === "pagehide_flush" || reason === "visibility_hidden_flush") {
        appendAuditEvent({ type: "sync:pagehide_flush_deferred", message: "session=" + TAB_ID + " runId=" + runId + " reason=" + reason + " stage=preflight dirty_preserved=true error=" + String(error && error.message || error || "") });
    ...
    if (verifiedHash !== uploadedHash) {
      ...
      if (typeof isRetryableVerifyFailure === "function" && isRetryableVerifyFailure(recheckError || ("expected=" + uploadedHash + ", actual=" + (recheck && currentRemoteHash(recheck) || String(verifiedHash))), reason)) {
        appendAuditEvent({ type: "sync:verify_mismatch_deferred", message: "session=" + TAB_ID + " runId=" + runId + " reason=" + String(reason || "") + " dirty_preserved=true" });
        var currentLocal = refreshLocalPayloadHash({ persist: false });
        markHashDirty(currentLocal.hash, "云端校验暂未确认，稍后自动重试", { runId });
