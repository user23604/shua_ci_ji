...
const APP_VERSION = "2026-07-26-final-stable-v3";
const APP_BUILD_ID = "2026-07-26-final-stable-v3";
...
function flushActivityForCloud(reason = "activity_flush") {
  ...
  // 先持久化 dirty，再清除 draft。即使浏览器在两步之间被系统强杀，重启后也会继续上传，
  // 避免仅 seconds 变化因业务 hash 排除秒数而被误判为已经同步。
  if (changed) {
    touchLocalSync();
...
function buildSyncDiagnosisText(extra = {}) {
  ...
  lines.push("localRecoverySource：" + (syncState.localRecoverySource || ""));
  lines.push("localRecoverySetAt：" + (syncState.localRecoverySetAt || ""));
  lines.push("localRecoveryCheckpointHash：" + String(syncState.localRecoveryCheckpointHash || "").slice(0, 16));
...
function showSyncProblemDialog(problem) {
  ...
  dialog.addEventListener("click", function(event) {
    ...
    if (action === "ignore_empty_backup") {
      clearLocalRecoveryLock("用户确认忽略空备份并继续同步", { userConfirmed: true });
      closeSyncProblemDialog(key);
...
function renderSyncDiagnostics() {
  ...
  var retryText = Number.isFinite(retryAtMs) && retryAtMs > Date.now() ? formatLocalDateTime(syncState.nextRetryAt) : "无等待";
...
function markHashCleanFromRemote(remote, payloadHash, status, options = {}) {
  if (isStaleSyncRun(options.runId)) return false;
  state.syncHashState = ensureHashSyncState(state.syncHashState);
  if (state.syncHashState.localRecoveryRequired && options.recoveryVerified !== true) {
    appendAuditEvent({ type: "recovery:mark_clean_blocked", message: "runId=" + String(options.runId || "") + " hash=" + String(payloadHash || "").slice(0, 8) });
    return false;
  }
  const now = beijingISOString();
...
function captureRemoteApplyTransaction() {
  ...
  return {
    keys: keys,
    items: items,
    businessHash: businessPayloadHash(collectSyncPayload()),
...
function rollbackRemoteApplyTransaction(snapshot, options = {}) {
  ...
  try {
    ...
    var restoredHash = businessPayloadHash(collectSyncPayload());
    if (snapshot.businessHash && restoredHash !== snapshot.businessHash) {
      ok = false;
      rollbackError = new Error("回滚后业务哈希不一致：expected=" + snapshot.businessHash + ", actual=" + restoredHash);
    }
  } catch (error) {
    ok = false;
    rollbackError = error;
  ...
  if (!ok) {
    var technical = String(rollbackError && rollbackError.message || rollbackError || "未知错误");
    state.syncHashState = ensureHashSyncState(state.syncHashState);
    state.syncHashState.lastBackupError = "本地回滚失败：" + technical;
    setLocalRecoveryRequired("本地事务回滚失败，当前数据可能处于部分新、部分旧的混合状态。已停止云同步，请使用 rescue.html 从明确备份恢复。", [], {
      runId: options.runId,
      source: "transaction_rollback_failed",
      checkpointHash: snapshot.businessHash || "",
      errorKind: "local_rollback_failed",
      stage: "local_rollback",
      technical: technical
    });
  }
  ...
  appendAuditEvent({
    ...
    message: "session=" + TAB_ID + " runId=" + String(options.runId || "") + " reason=" + String(options.reason || "") + " checkpoint=" + String(snapshot.businessHash || "").slice(0, 8)
  });
  return ok;
}
...
function applyRemotePayloadSafely(payload, options = {}) {
  ...
  try {
    ...
    if (!allSaved) {
      var rolledBackAfterWriteFailure = rollbackRemoteApplyTransaction(transactionSnapshot, { runId: options.runId, reason: "storage_write_failed" });
      recordHashSyncFailure(rolledBackAfterWriteFailure
        ? "云端数据写入本地存储失败，已自动回滚到写入前状态并保留覆盖前备份"
        : "云端数据写入失败且本地回滚未完成，已锁定云同步并要求人工恢复", {
        errorKind: rolledBackAfterWriteFailure ? "local_storage_write_failed" : "local_rollback_failed",
        runId: options.runId,
        banner: true,
        dialog: true,
        stage: rolledBackAfterWriteFailure ? "local_apply" : "local_rollback"
      });
      return false;
    }
    ...
    if (!allSaved) {
      var rolledBackAfterFinalizeFailure = rollbackRemoteApplyTransaction(transactionSnapshot, { runId: options.runId, reason: "pending_or_meta_write_failed" });
      recordHashSyncFailure(rolledBackAfterFinalizeFailure
        ? "云端数据写入收尾失败，已自动回滚到写入前状态"
        : "云端数据写入收尾失败且本地回滚未完成，已锁定云同步并要求人工恢复", {
        errorKind: rolledBackAfterFinalizeFailure ? "local_storage_write_failed" : "local_rollback_failed",
        runId: options.runId,
        banner: true,
        dialog: true,
        stage: rolledBackAfterFinalizeFailure ? "local_apply_finalize" : "local_rollback"
      });
      return false;
    }
    ...
    if (afterHash !== expectedHash) {
      var rolledBackAfterHashFailure = rollbackRemoteApplyTransaction(transactionSnapshot, { runId: options.runId, reason: "hash_verify_failed" });
      recordHashSyncFailure(rolledBackAfterHashFailure
        ? "本地数据写入后校验失败，已自动回滚到写入前状态并保留覆盖前备份"
        : "本地数据写入后校验失败且回滚未完成，已锁定云同步并要求人工恢复", {
        errorKind: rolledBackAfterHashFailure ? "local_apply_verify_failed" : "local_rollback_failed",
        runId: options.runId,
        banner: true,
        dialog: true,
        stage: rolledBackAfterHashFailure ? "local_verify" : "local_rollback",
...
async function syncTickInternal({ reason = "heartbeat", keepalive = false, bypassBackoff = false } = {}) {
  ...
  try { processPendingStorageReadIssues({ source: "pre_sync" }); } catch (_) {}
  if (ensureHashSyncState(state.syncHashState).localRecoveryRequired) {
    const recoveryBeforeNetwork = tryRestoreFromBackupIfPayloadEmpty({});
    if (recoveryBeforeNetwork.status === "restore_failed") {
      enforceLocalRecoveryGuardAtStartup();
      updateSyncIndicator();
      return false;
    }
  }
...
function canMarkCloudOkAfterVerify({ uploadedHash, currentHash, runId } = {}) {
  ...
  var syncState = ensureHashSyncState(state.syncHashState);
  // 新鲜、完整的写后读回验证是新的事实，应清除旧 auth/merge blocker。
  // 唯一本地安全例外是仍处于人工恢复保护状态。
  if (syncState.localRecoveryRequired) return false;
  return true;
}
...
function finalizeVerifiedPatch({ uploadedPayload, uploadedHash, verifiedRemote, runId }) {
  ...
  var markedClean = markHashCleanFromRemote(verifiedRemote, uploadedHash, "cloud_ok", { runId: runId, remoteVerified: true });
  return markedClean === true;
}
