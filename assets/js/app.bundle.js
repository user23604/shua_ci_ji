...
const APP_BUILD_ID = "2026-09-03-flash-unit-quick-switch-v5";
...
function handleStorageQuotaExceeded(key, value, options = {}) {
  pruneStorageForQuota({ ...options, aggressive: false });
  try {
    localStorage.setItem(key, value);
    return true;
  } catch (firstError) {
    if (options.allowAggressiveBackupEviction === false) {
      if (options.silentFailure === true) {
        try {
          if (typeof state !== "undefined" && typeof ensureHashSyncState === "function") {
            state.syncHashState = ensureHashSyncState(state.syncHashState);
            state.syncHashState.lastBackupError = "本地浏览器存储空间不足，已跳过非关键本地备份：" + String(key || "");
          }
        } catch (_) {}
        return false;
      }
      return handleStorageWriteFailure(key, firstError, options);
    }
    pruneStorageForQuota({ ...options, aggressive: true, dropAllBackups: true });
    try {
      localStorage.setItem(key, value);
      return true;
    } catch (err) {
...
function pruneStorageForQuota(options = {}) {
  try {
    if (typeof compactExistingLocalBackupCopies === "function") compactExistingLocalBackupCopies();
  } catch (_) {}
  try { localStorage.removeItem(SYNC_AUDIT_KEY); } catch (_) {}
  try {
    if (options.dropAllBackups && typeof clearRedundantLocalBackupCopiesForQuota === "function") {
      clearRedundantLocalBackupCopiesForQuota();
    } else if (typeof pruneLocalBackupCopiesToBudget === "function") {
      pruneLocalBackupCopiesToBudget(LOCAL_BACKUP_SOFT_BUDGET_BYTES, { keepOneRecovery: true });
    } else {
...
const BUSINESS_HASH_SCHEMA_VERSION = 4;
...
function normalizeBookSettingValues(book, values) {
  ...
  return {
    ...
    preferStandardAudio: typeof source.preferStandardAudio === "boolean" ? source.preferStandardAudio : DEFAULT_SETTINGS.preferStandardAudio,
...
function persistSyncMeta() {
  state.syncMeta = ensureSyncMeta(state.syncMeta);
  return saveJson(SYNC_META_KEY, state.syncMeta, { allowAggressiveBackupEviction: false, silentFailure: true });
}
...
function persistHashSyncState() {
  state.syncHashState = ensureHashSyncState(state.syncHashState);
  try {
    var saved = saveJson(HASH_SYNC_STATE_KEY, state.syncHashState, { allowAggressiveBackupEviction: false, silentFailure: true });
...
function hasBusinessData(payload) {
  ...
  return countProgressRecords(p) > 0 ||
    ...
    hasRoundArchiveBusinessData(p.round, p.archives) ||
...
function renderFlashcard({ touchProgress = false, progressReason = "render" } = {}) {
  ...
  scheduleWordTimers();
  if (typeof primePronunciationForCurrentStudy === "function") primePronunciationForCurrentStudy();
...
async function scheduleWordTimers() {
  ...
  if (hasEnSpeech) {
    let spoken = false;
    if (typeof playStandardEnglishAudio === "function" && standardPronunciationEnabled()) {
      spoken = await playStandardEnglishAudio(word.en, token);
    }
    if (!isPlaybackToken(token)) return;
    if (!spoken && speechAvailable) spoken = await speakWithHighlight(word.en, "en-US", "en", token);
    if (!isPlaybackToken(token)) return;
...
function setSpeechPhase(phase, rate, sourceLabel = "") {
  state.speechPhase = phase;
  state.activeZhIndex = -1;
  const en = document.getElementById("wordEn");
  const status = document.getElementById("speechStatus");
  if (en) en.classList.toggle("is-speaking", phase === "en");
  if (status) status.textContent = `${sourceLabel || (phase === "en" ? "朗读英文" : "朗读义项")} · ${formatRate(rate)}x`;
...
function unlockSpeech() {
  if (typeof unlockPronunciationAudio === "function") unlockPronunciationAudio();
  if (!("speechSynthesis" in window)) return;
...
function renderArchiveDrawer() {
  ...
  const list = state.archiveTab === "history"
    ? renderRoundArchiveHistory()
    : (groups.length ? groups.map(renderArchiveGroup).join("") : `<div class="status">暂无记录。</div>`);
  const selectionHint = state.archiveTab === "history"
    ? "历史轮次只读保留；归档当前轮后开启全新一轮"
    : state.archiveSelectionMode === "unit"
      ? "点选 Unit 后开始组合刷词"
    ...
    <div class="archive-backdrop" id="archiveBackdrop">
      <aside class="archive-drawer" role="dialog" aria-modal="true">
        ...
        <div class="tabs archive-tabs">
          ...
          <button class="tab ${state.archiveTab === "history" ? "is-active" : ""}" data-archive-tab="history" type="button">历史归档</button>
        </div>
        <div class="archive-body">${status}${list}</div>
      </aside>
      ${renderRoundArchiveForm()}
    </div>
  `;
}
...
function bindArchiveEvents() {
  ...
  if (typeof bindRoundArchiveUiEvents === "function") bindRoundArchiveUiEvents();
}
