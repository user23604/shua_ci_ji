...
const APP_VERSION = "2026-06-20-p2";
...
const SYNC_STUCK_TIMEOUT_MS = 35000;
const SYNC_FAILURE_BANNER_MS = 1800;
const SYNC_FAILURE_BANNER_DEDUPE_MS = 3000;
const SYNC_PROBLEM_MODAL_DEDUPE_MS = 8000;
...
const state = {
  ...
  lastSyncBannerKey: "",
  lastSyncBannerAt: 0,
  lastSyncProblemModalKey: "",
  lastSyncProblemModalAt: 0,
  isSyncing: false,
...
function setHashSyncStatus(status, message = "") {
  ...
  if (message && ["error", "conflict", "invalid_config", "read_only"].includes(status)) {
    state.syncHashState.lastSyncError = message;
  }
...
function formatLocalDateTime(isoOrDate) {
  if (!isoOrDate) return "无";
  try {
    const date = isoOrDate instanceof Date ? isoOrDate : new Date(isoOrDate);
    if (isNaN(date.getTime())) return String(isoOrDate || "无");
    ...
    return String(isoOrDate || "无");
  }
}

function formatDiagnosticDateTime(iso) {
  if (!iso) return "无";
  const local = formatLocalDateTime(iso);
  return local === String(iso) ? String(iso) : local + "（UTC " + String(iso) + "）";
}

function getSyncToneClass(status) {
  if (status === "syncing") return "is-syncing";
  if (status === "cloud_saved") return "is-ok";
  if (status === "error" || status === "conflict" || status === "invalid_config") return "is-error";
  if (status === "dirty" || status === "local_only" || status === "read_only") return "is-warn";
  return "";
}

function renderSyncClassName(status) {
  const semantic = "is-" + normalizeSyncStatus(status);
  const tone = getSyncToneClass(status);
  return ["cloud-sync-indicator", semantic, tone].filter(Boolean).join(" ");
}

function showSyncFailureBanner(message, title = "云同步失败") {
  if (typeof document === "undefined" || !document.body) return;
  const safeMessage = String(message || "同步没有成功，本地数据已保留，稍后会自动重试。");
  const dedupeKey = title + "|" + safeMessage;
  const now = Date.now();
  if (state.lastSyncBannerKey === dedupeKey && now - state.lastSyncBannerAt < SYNC_FAILURE_BANNER_DEDUPE_MS) return;
  state.lastSyncBannerKey = dedupeKey;
  state.lastSyncBannerAt = now;

  let banner = document.getElementById("syncFailureBanner");
  if (!banner) {
  ...
  banner.className = "sync-failure-banner is-visible";
  banner.innerHTML = '<strong>' + escapeHtml(title) + '</strong><span>' + escapeHtml(safeMessage) + '</span>';
  if (state.syncBannerTimer) clearTimeout(state.syncBannerTimer);
  state.syncBannerTimer = setTimeout(function() {
    banner.classList.remove("is-visible");
  }, SYNC_FAILURE_BANNER_MS);
}

function buildSyncDiagnosisText(problemMessage = "", problemTitle = "云同步问题") {
  ...
  var lines = [];
  lines.push("刷词机同步问题诊断");
  lines.push("================================");
  lines.push("问题标题：" + (problemTitle || "云同步问题"));
  lines.push("问题详情：" + (problemMessage || syncState.lastSyncError || meta.lastSyncErrorMessage || "无"));
  lines.push("导出时间：" + formatDiagnosticDateTime(new Date().toISOString()));
  ...
  lines.push("正在同步：" + (state.isSyncing ? "是" : "否"));
  lines.push("同步开始时间：" + formatDiagnosticDateTime(state.syncStartedAt));
  ...
  lines.push("dirtySince：" + formatDiagnosticDateTime(syncState.dirtySince));
  lines.push("最近成功 Push：" + formatDiagnosticDateTime(syncState.lastSuccessfulPushAt || meta.lastSuccessfulPushAt));
  lines.push("最近成功 Pull：" + formatDiagnosticDateTime(syncState.lastSuccessfulPullAt || meta.lastSuccessfulPullAt));
  lines.push("最近尝试同步：" + formatDiagnosticDateTime(meta.lastSyncAttemptAt));
  lines.push("最近错误时间：" + formatDiagnosticDateTime(meta.lastSyncErrorAt));
  lines.push("旧 pendingOps：" + opsCount + " 条（P0 已冻结，不再写入）");
  ...
  lines.push("本地最新快照：" + getLocalSnapshotTime());
  lines.push("今日每日备份：" + getDailyBackupTime());
  lines.push("备份索引数量：" + loadHashBackupIndex().length);
  if (syncState.lastBackupError) lines.push("备份写入错误：" + syncState.lastBackupError);
  return lines.join("\n");
}

function showSyncProblemModal(message, title = "云同步出现问题", options = {}) {
  if (typeof document === "undefined" || !document.body) return;
  const safeMessage = String(message || "同步没有成功，本地数据已保留，请截图或复制诊断信息反馈。");
  const dedupeKey = title + "|" + safeMessage;
  const now = Date.now();
  const force = options && options.force === true;
  const existing = document.getElementById("syncProblemModal");
  if (!force && !existing && state.lastSyncProblemModalKey === dedupeKey && now - state.lastSyncProblemModalAt < SYNC_PROBLEM_MODAL_DEDUPE_MS) {
    showSyncFailureBanner(safeMessage, title);
    return;
  }
  state.lastSyncProblemModalKey = dedupeKey;
  state.lastSyncProblemModalAt = now;
  showSyncFailureBanner(safeMessage, title);

  let modal = existing;
  if (!modal) {
    modal = document.createElement("div");
    modal.id = "syncProblemModal";
    modal.className = "sync-problem-modal";
    modal.setAttribute("role", "dialog");
    modal.setAttribute("aria-modal", "true");
    modal.setAttribute("aria-labelledby", "syncProblemModalTitle");
    document.body.appendChild(modal);
  }
  const report = buildSyncDiagnosisText(safeMessage, title);
  modal.className = "sync-problem-modal is-visible";
  modal.innerHTML = `
    <div class="sync-problem-modal__card">
      <div class="sync-problem-modal__header">
        <div>
          <h2 id="syncProblemModalTitle">${escapeHtml(title)}</h2>
          <p>本地数据已保留，但云端没有确认保存。请先不要清理浏览器网站数据。</p>
        </div>
        <button class="sync-problem-modal__x" id="syncProblemCloseX" type="button" aria-label="关闭">×</button>
      </div>
      <div class="sync-problem-modal__message">${escapeHtml(safeMessage)}</div>
      <label class="sync-problem-modal__label" for="syncProblemReport">可截图/复制给程序员的诊断信息</label>
      <textarea id="syncProblemReport" class="sync-problem-modal__report" readonly>${escapeHtml(report)}</textarea>
      <div class="sync-problem-modal__actions">
        <button class="btn btn--primary" id="copySyncProblemReport" type="button">复制诊断</button>
        <button class="btn btn--ghost" id="closeSyncProblemModal" type="button">我知道了</button>
      </div>
    </div>
  `;
  const close = function() { modal.classList.remove("is-visible"); };
  const x = document.getElementById("syncProblemCloseX");
  const closeBtn = document.getElementById("closeSyncProblemModal");
  const copyBtn = document.getElementById("copySyncProblemReport");
  if (x) x.addEventListener("click", close);
  if (closeBtn) closeBtn.addEventListener("click", close);
  if (copyBtn) copyBtn.addEventListener("click", function() {
    const text = buildSyncDiagnosisText(safeMessage, title);
    const textarea = document.getElementById("syncProblemReport");
    if (textarea) textarea.value = text;
    const fallbackCopy = function() {
      if (textarea) {
        textarea.focus();
        textarea.select();
        try { document.execCommand("copy"); copyBtn.textContent = "已复制"; } catch (_) { copyBtn.textContent = "请手动复制"; }
      }
    };
...
function isSyncStale() {
  ...
  const started = Date.parse(state.syncStartedAt || "");
  return Number.isFinite(started) && Date.now() - started > SYNC_STUCK_TIMEOUT_MS;
}

function ensureSyncNotStuck() {
  if (!isSyncStale()) return false;
  const startedText = formatDiagnosticDateTime(state.syncStartedAt);
  state.isSyncing = false;
  state.syncStartedAt = "";
  recordHashSyncFailure("同步流程超过 " + Math.round(SYNC_STUCK_TIMEOUT_MS / 1000) + " 秒仍未结束，已自动解除“同步中”锁。开始时间：" + startedText + "。本地数据已保留，将等待下一轮重试。");
  return true;
}

async function syncFetch(url, options = {}, label = "云同步请求", timeoutMs = SYNC_REQUEST_TIMEOUT_MS) {
  let timeoutId = 0;
  let controller = null;
  let timedOut = false;
  ...
  const timeoutPromise = new Promise(function(_, reject) {
    timeoutId = setTimeout(function() {
      ...
      reject(new Error(label + "超时：超过 " + Math.round(timeoutMs / 1000) + " 秒没有响应，已停止本轮同步。本地数据已保留。"));
    }, timeoutMs);
  });
  try {
    ...
    if (timedOut || error?.name === "AbortError") {
      throw new Error(label + "超时：超过 " + Math.round(timeoutMs / 1000) + " 秒没有响应，已停止本轮同步。本地数据已保留。");
    }
    throw error;
  } finally {
...
function recordHashSyncFailure(message) {
  ...
  showSyncProblemModal(state.syncHashState.lastSyncError, "云同步失败");
  updateSyncIndicator();
}
...
function requestSyncAfterLocalChange(reason = "change") {
  ...
  state.autoPushDebounceTimer = setTimeout(function() {
    ...
    syncTick({ reason: "local_change:" + reason, bypassBackoff: true });
...
function markLocalDirtyAfterBusinessWrite(reason = "change") {
  ...
  updateSyncIndicator();
  requestSyncAfterLocalChange(reason);
}
...
function renderSyncIndicator() {
  ...
  const timeText = info.status === "cloud_saved" && state.syncMeta.lastCloudSaveConfirmedAt
    ...
    : "";
  return `
    <div class="${renderSyncClassName(info.status)}" id="cloudSyncIndicator"
...
function updateSyncIndicatorDOM(info) {
  ...
  if (indicator) {
    indicator.className = renderSyncClassName(info.status);
...
function tryRestoreFromBackupIfPayloadEmpty() {
  ...
  try {
    var idx = loadJson("vocab_machine_backup_index_v1", { items: [] });
    var idxItems = Array.isArray(idx) ? idx : (Array.isArray(idx.items) ? idx.items : []);
    var startups = idxItems.filter(function(e) { return e && (e.kind === "startup" || e.tag === "startup"); });
  ...
  persistHashSyncState();
  var recoveryMessage = "本地备份数据无法解析恢复，请打开 rescue.html 手动导出备份再联系处理。备份数据仍保留在浏览器中。";
  setHashSyncStatus("error", recoveryMessage);
  showSyncProblemModal(recoveryMessage, "本地备份恢复失败", { force: true });
...
function renderSyncDiagnostics() {
  ...
  lines.push('<div>dirtySince：' + escapeHtml(formatDiagnosticDateTime(syncState.dirtySince)) + '</div>');
  lines.push('<div>最近成功 Push：' + escapeHtml(formatDiagnosticDateTime(syncState.lastSuccessfulPushAt || meta.lastSuccessfulPushAt)) + '</div>');
  lines.push('<div>最近成功 Pull：' + escapeHtml(formatDiagnosticDateTime(syncState.lastSuccessfulPullAt || meta.lastSuccessfulPullAt)) + '</div>');
  ...
  lines.push('<div>关键备份：' + backups.length + ' 条；最新本地快照：' + escapeHtml(formatDiagnosticDateTime(getLocalSnapshotTimeRaw())) + '</div>');
  if (backups.length > 0) {
    ...
    lines.push('<div style="font-size:11px;color:#94a3b8;">最近备份：' + recentBackups.map(function(b) {
      return escapeHtml((b.kind || b.tag || "?") + " " + formatDiagnosticDateTime(b.savedAt));
    }).join("；") + '</div>');
  }
  lines.push('<div>今日备份：' + escapeHtml(formatDiagnosticDateTime(getDailyBackupTimeRaw())) + '</div>');
...
function getLocalSnapshotTimeRaw() {
  try {
...
function getLocalSnapshotTime() {
  return formatDiagnosticDateTime(getLocalSnapshotTimeRaw());
}

function getDailyBackupTimeRaw() {
  try {
...
function getDailyBackupTime() {
  return formatDiagnosticDateTime(getDailyBackupTimeRaw());
}
...
async function testAndSaveCloudConfig() {
  ...
  if (!validation.ok) {
    var validationMessage = validation.errors.join("；");
    if (statusEl) {
      statusEl.textContent = validationMessage;
      statusEl.className = "status status--error";
    }
    showSyncProblemModal(validationMessage, "云同步配置错误", { force: true });
    return;
  }
  ...
  var getResponse;
  try {
    getResponse = await syncFetch(getUrl, {
      headers: { Authorization: "Bearer " + draft.token, Accept: "application/vnd.github+json" }
    }, "配置测试 GET Gist");
  } catch (e) {
    var msg = syncErrorMessage(e);
    if (statusEl) { statusEl.textContent = "❌ " + msg; statusEl.className = "status status--error"; }
    showSyncProblemModal(msg, "云同步配置测试失败", { force: true });
    return;
  }

  if (getResponse.status === 401 || getResponse.status === 403) {
    // 尝试公开访问
    var publicResp = await syncFetch(getUrl, { headers: { Accept: "application/vnd.github+json" } }, "配置测试公开读取").catch(function() { return null; });
    if (publicResp && publicResp.ok) {
      var readOnlyMessage = "PAT 无效，但 Gist 是公开的——只能读取，无法上传。请重新生成有 Gist 写入权限的 PAT。";
      if (statusEl) { statusEl.textContent = "❌ " + readOnlyMessage; statusEl.className = "status status--error"; }
      showSyncProblemModal(readOnlyMessage, "云同步配置测试失败", { force: true });
    } else {
      var invalidPatMessage = "PAT 无效、已过期或没有此 Gist 的访问权限。";
      if (statusEl) { statusEl.textContent = "❌ " + invalidPatMessage; statusEl.className = "status status--error"; }
      showSyncProblemModal(invalidPatMessage, "云同步配置测试失败", { force: true });
    }
    return;
  }

  if (!getResponse.ok) {
    var getFailMessage = "无法访问 Gist：HTTP " + getResponse.status;
    if (statusEl) { statusEl.textContent = "❌ " + getFailMessage; statusEl.className = "status status--error"; }
    showSyncProblemModal(getFailMessage, "云同步配置测试失败", { force: true });
    return;
  }
  ...
  var patchResponse;
  try {
    patchResponse = await syncFetch(getUrl, {
      method: "PATCH",
      headers: {
    ...
    var msg = syncErrorMessage(e);
    var writeFailMessage = "写权限测试失败：" + msg;
    if (statusEl) { statusEl.textContent = "❌ " + writeFailMessage; statusEl.className = "status status--error"; }
    showSyncProblemModal(writeFailMessage, "云同步配置测试失败", { force: true });
    return;
  }

  if (!patchResponse.ok) {
    var patchFailMessage = patchResponse.status === 403
      ? "PAT 没有 Gist 写入权限（只能读）。请更新 PAT 权限范围。"
      : "写权限测试失败：HTTP " + patchResponse.status;
    if (statusEl) { statusEl.textContent = "❌ " + patchFailMessage; statusEl.className = "status status--error"; }
    showSyncProblemModal(patchFailMessage, "云同步配置测试失败", { force: true });
    return;
  }
...
function exportDiagnosisSummary() {
  ...
  lines.push("导出时间：" + formatDiagnosticDateTime(exportedAt));
  lines.push("应用版本：" + APP_VERSION);
  lines.push("同步状态：" + info.status + " - " + (info.detail || ""));
  lines.push("Gist ID：" + gistMasked);
  lines.push("PAT 格式：" + (config.ok ? "通过" : "失败：" + config.errors.join("；")));
...
function computeSyncStatus() {
  ...
  if (state.isSyncing) {
    if (isSyncStale()) return { status: "error", detail: "同步超过 " + Math.round(SYNC_STUCK_TIMEOUT_MS / 1000) + " 秒未结束，等待自动解锁" };
    return { status: "syncing", detail: "正在同步" };
  }
...
async function fetchGistMetadata() {
  ...
  const authResponse = await syncFetch(url, {
    headers: {
  ...
  if (authResponse.status === 401 || authResponse.status === 403) {
    const publicResponse = await syncFetch(url, {
      headers: {
        Accept: "application/vnd.github+json"
      }
    }, "公开 GET Gist 元数据");
...
async function readGistFileContent(file, { unauthenticated = false } = {}) {
  ...
  const response = await syncFetch(file.raw_url, { headers }, "读取 Gist 文件内容");
...
function savedCloudConfigGate() {
  ...
  if (!validation.ok) {
    ...
    var invalidMessage = validation.errors.join("；");
    state.syncMeta.lastSyncErrorMessage = invalidMessage;
    persistSyncMeta();
    setHashSyncStatus("invalid_config", invalidMessage);
    if (state.cloud.token || state.cloud.gistId) {
      showSyncProblemModal(invalidMessage, "云同步配置错误");
    }
...
function setReadOnlySyncState(message) {
  ...
  persistSyncMeta();
  showSyncProblemModal(state.syncMeta.lastSyncErrorMessage, "云同步只读");
...
async function initializeP0Sync({ reason = "init" } = {}) {
  ...
  state.syncMeta.lastSyncAttemptAt = state.syncStartedAt;
  persistSyncMeta();
...
async function syncBranchMerge({ remote, remotePayload, local, keepalive, reason }) {
  ...
  if (!validateSyncPayload(normalized)) {
    ...
    persistHashSyncState();
    showSyncProblemModal(state.syncHashState.lastSyncError, "云同步自动合并失败", { force: true });
    updateSyncIndicator();
    return false;
  }
...
async function patchBusinessPayloadToGist(payload, { remote, keepalive = false } = {}) {
  ...
  appendAuditEvent({ type: "push:start", message: "开始上传 sync.json" });
  ...
  let response;
  try {
    response = await syncFetch("https://api.github.com/gists/" + encodeURIComponent(state.cloud.gistId), {
      method: "PATCH",
      keepalive,
      headers: {
        Authorization: "Bearer " + state.cloud.token,
  ...
  appendAuditEvent({ type: "push:succeeded", message: "PATCH 后 GET 校验 hash 匹配", remoteVersion: verified.remoteVersion || "" });
...
function enterSafeConflictMode(message) {
  ...
  showSyncProblemModal(message || "同步已安全阻断", "云同步已阻断", { force: true });
  if (state.view === "setup") {
...
function syncErrorMessage(error) {
  ...
  if (/超时|timeout|AbortError/i.test(raw)) return raw;
  return raw;
}
